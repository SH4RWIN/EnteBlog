...
- **CORS Role:** For non-simple requests (`PUT`, `DELETE`, custom `Content-Type`), the browser sends a **Preflight Request (`OPTIONS`)**. If the malicious origin is not explicitly allowed by the API server, the browser blocks the dangerous action request entirely.![](https://)